-- QB10 -- Commitment Loss : PREFLIGHT / data-readiness check (DuckDB)
--
-- Run this BEFORE the detector. It answers one question: can this data even answer
-- the question QB10 asks?
--
...
--   .read queries/qb10-commitment-loss/preflight.duckdb.sql

WITH win AS (
    SELECT *
    FROM bill
    WHERE CAST(chargeperiodstart AS DATE) >= (CURRENT_DATE - INTERVAL 37 DAY)
),
usable AS (   -- the row filter every QB detector applies before its own logic
    SELECT *
    FROM win
...
SELECT check_name, observed, needed, status FROM (
    SELECT 1 AS seq,
           'rows in the last 37 days'                                AS check_name,
    ...
    UNION ALL
    SELECT 5,
           'column populated: commitmentdiscountid',
           COALESCE((SELECT ROUND(100.0 * COUNT(commitmentdiscountid) / NULLIF(COUNT(*), 0), 1) FROM usable), 0)::VARCHAR || '%',
           '> 0%',
           CASE WHEN COALESCE((SELECT COUNT(commitmentdiscountid) FROM usable), 0) > 0
                THEN 'PASS' ELSE 'FAIL' END

    UNION ALL
    SELECT 6,
           'column populated: commitmentdiscountstatus',
           COALESCE((SELECT ROUND(100.0 * COUNT(commitmentdiscountstatus) / NULLIF(COUNT(*), 0), 1) FROM usable), 0)::VARCHAR || '%',
           '> 0%',
           CASE WHEN COALESCE((SELECT COUNT(commitmentdiscountstatus) FROM usable), 0) > 0
                THEN 'PASS' ELSE 'FAIL' END
