-- QB04 -- Utilization Ratio : PREFLIGHT / data-readiness check (DuckDB)
--
-- Run this BEFORE the detector. It answers one question: can this data even answer
-- the question QB04 asks?
--
...
--   .read queries/qb04-utilization-ratio/preflight.duckdb.sql
...
SELECT check_name, observed, needed, status FROM (
    ...
    SELECT 2,
           ...
           '>= 7',
           CASE WHEN (SELECT COUNT(DISTINCT CAST(chargeperiodstart AS DATE)) FROM win) >= 7
    ...
    UNION ALL
    SELECT 8,
           'column populated: consumedunit',
           COALESCE((SELECT ROUND(100.0 * COUNT(consumedunit) / NULLIF(COUNT(*), 0), 1) FROM usable), 0)::VARCHAR || '%',
           '> 0%',
           CASE WHEN COALESCE((SELECT COUNT(consumedunit) FROM usable), 0) > 0
                THEN 'PASS' ELSE 'FAIL' END
