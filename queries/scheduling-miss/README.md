...
```sql
.read queries/scheduling-miss/preflight.duckdb.sql    -- CHECK: can this data answer the question?
.read queries/scheduling-miss/diagnostic.duckdb.sql   -- LEARN: the ranked field, rules as pass/fail flags
.read queries/scheduling-miss/query.duckdb.sql        -- TRUST: what actually fires
```
...
- This pattern is distinct from Idle Developer Resources (Idle Developer Resource): Scheduling Miss catches resources
  with high absolute weekend COST (batch jobs, always-on services). Idle Developer Resources catches resources
