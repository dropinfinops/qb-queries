-- QB16 -- Idle Model Endpoint : PREFLIGHT / data-readiness check (DuckDB)
--
-- Run this BEFORE the detector. It answers one question: can this data even answer
-- the question QB16 asks?
--
...
--   .read queries/qb16-idle-model-endpoint/preflight.duckdb.sql

WITH win AS (
    SELECT *
    FROM bill
    WHERE CAST(chargeperiodstart AS DATE) >= (CURRENT_DATE - INTERVAL 7 DAY)
),
...
SELECT check_name, observed, needed, status FROM (
    SELECT 1 AS seq,
           'rows in the last 7 days'                                AS check_name,
