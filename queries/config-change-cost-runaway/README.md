...
```sql
.read queries/config-change-cost-runaway/preflight.duckdb.sql    -- CHECK: can this data answer the question?
.read queries/config-change-cost-runaway/diagnostic.duckdb.sql   -- LEARN: the ranked field, rules as pass/fail flags
.read queries/config-change-cost-runaway/query.duckdb.sql        -- TRUST: what actually fires
```
