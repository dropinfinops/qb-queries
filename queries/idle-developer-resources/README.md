...
```sql
.read queries/idle-developer-resources/preflight.duckdb.sql    -- CHECK: can this data answer the question?
.read queries/idle-developer-resources/diagnostic.duckdb.sql   -- LEARN: the ranked field, rules as pass/fail flags
.read queries/idle-developer-resources/query.duckdb.sql        -- TRUST: what actually fires
```
...
- This pattern is related to but distinct from Scheduling Miss (Scheduling Miss): Scheduling Miss looks at
  absolute weekend cost relative to weekday cost. Idle Developer Resources looks at activity-to-cost
  efficiency — a resource can pass Scheduling Miss (low weekend cost) but fail Idle Developer Resources (activity is
