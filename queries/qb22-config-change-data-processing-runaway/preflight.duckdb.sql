-- QB22 -- Config-Change Data-Processing Runaway : PREFLIGHT / data-readiness check (DuckDB)
--
-- Run this BEFORE the detector. It answers one question: can this data even answer
-- the question QB22 asks?
--
...
WITH win AS (
    SELECT *
    FROM bill
    WHERE CAST(chargeperiodstart AS DATE) >= (CURRENT_DATE - INTERVAL 60 DAY)
),
...
SELECT check_name, observed, needed, status FROM (
    SELECT 1 AS seq,
           'rows in the last 60 days'                                AS check_name,
    ...
    SELECT 2,
           ...
           '>= 44',
           CASE WHEN (SELECT COUNT(DISTINCT CAST(chargeperiodstart AS DATE)) FROM win) >= 44
    ...
    UNION ALL
    SELECT 8,
           'column populated: x_usagetype',
           COALESCE((SELECT ROUND(100.0 * COUNT(x_usagetype) / NULLIF(COUNT(*), 0), 1) FROM usable), 0)::VARCHAR || '%',
           '> 0%',
           CASE WHEN COALESCE((SELECT COUNT(x_usagetype) FROM usable), 0) > 0
                THEN 'PASS' ELSE 'FAIL' END
